...
public class WithSemaphore {
  ...
  public class TreeNode {
    int val;
    Semaphore locked = new Semaphore(1, true);
    TreeNode parent;
  ...
  public static boolean lock(TreeNode node) {
    ...
    // if India is locked then descendent childs are not even able to come here bcoz
    // we already locked parent
    signal(node.locked);
  ...
  public boolean unLock(TreeNode node) {
    ...
    while (true) {
      ...
      // after completing the writing section we increase db variable so other writers
      // can come
      signal(db);
